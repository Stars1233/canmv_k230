...
#define LV_FREETYPE_PATH_MAX_LEN    (256)
...
typedef struct _lv_freetype_font_dsc_t {
    ...
    uint16_t size;
    uint16_t style;
    char pathname[LV_FREETYPE_PATH_MAX_LEN];
...
lv_font_t * lv_freetype_font_create(const char * pathname, uint16_t size, uint16_t style)
{
    ...
    if(pathname_len >= LV_FREETYPE_PATH_MAX_LEN) {
        LV_LOG_ERROR("pathname is too large");
        return NULL;
    }

    size_t need_size = sizeof(lv_freetype_font_dsc_t) + sizeof(lv_font_t);
    void *buffer = lv_malloc(need_size);
    LV_ASSERT_MALLOC(buffer);
    if(!buffer) {
        LV_LOG_ERROR("malloc failed for lv_freetype_font_dsc");
        return NULL;
    }
    lv_memzero(buffer, need_size);

    lv_freetype_font_dsc_t * dsc = (lv_freetype_font_dsc_t *)((uint8_t *)buffer + sizeof(lv_font_t));
    dsc->font = (lv_font_t *)buffer;

    lv_strncpy(dsc->pathname, pathname, LV_FREETYPE_PATH_MAX_LEN);
