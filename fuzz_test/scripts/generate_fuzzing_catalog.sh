...
priority_for() {
    ...
    case "$rel" in
        port/omv/common/array.c|port/omv/common/ringbuf.c|port/omv/common/ff_wrapper.c|port/omv/alloc/unaligned_memcpy.c)
            echo "High" ;;
        port/omv/alloc/*|port/omv/common/*|*.py)
            echo "Medium" ;;
        *)
            echo "Low" ;;
    esac
...

{
    ...
    echo "3. Python parser/import fuzzing through a host MicroPython build"
    echo "4. HAL-bound modules once dedicated fake backends exist"
} > "${OUT_FILE}"
